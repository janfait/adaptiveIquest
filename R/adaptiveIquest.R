...
distribution <- function(rid=NULL,q=NULL,choices=NULL,selected=NULL,demo=NULL,mult=F){
  ...
  if(is.null(rid)|is.null(q)){
    ...
    fname <- paste("http://192.168.10.181/ocpu/library/adaptiveIquest/data/rid",rid,"_",q,"_",par,".csv",sep="")
    fname_v <- paste("http://192.168.10.181/ocpu/library/adaptiveIquest/data/rid",rid,"_",q,"_",par,"variance.csv",sep="")
  }
